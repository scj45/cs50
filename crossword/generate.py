...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        if overlap:
            ...
            for xval in self.domains[x]:
                ...
                if not valid:
                    remove.append(xval)
            print(f"Values to remove from {x}: {remove}")
            for val in remove:
        ...
        for val in self.domains[var]:
            ...
            for neighbour in neighbours:
                if val in self.domains[neighbour]:
                    numneighbours += 1
            values.update({val: numneighbours})

        sortedval = dict(sorted(values.items(), key=lambda x: x[1]))
        sort = list(sortedval.keys())
    ...
    def select_unassigned_variable(self, assignment):
        ...
        sav = [variable for variable in unassigned]

        sav.sort(key=lambda x: (len(self.domains[x]), -len(self.crossword.neighbors(x))))
        return sav[0]
